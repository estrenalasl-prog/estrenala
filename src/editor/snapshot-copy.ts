...
// Copia el snapshot actual a uno nuevo (tipo por defecto "edit"): las páginas .html
// pasan por `transformar` (null = sin cambios, se copian los bytes originales sin
// recodificar); `extras` añade archivos (p. ej. assets a wc-uploads/) y `excluir` omite
// rutas relativas del snapshot origen (p. ej. para reemplazarlas vía `extras`, sin doble
// escritura). Si el alta del snapshot en BD falla, se limpia el storage escrito
// (compensación). Mecánica compartida por saveEdits y por las herramientas del sitio.
export async function crearSnapshotEditado(
  ...
  try {
    ...
    for (const key of keys) {
      ...
      if (input.excluir?.has(rel)) continue;
  ...
  try {
    await deps.store.createSnapshot({
      ...
      tipo: input.tipo ?? "edit", storagePrefix: newPrefix, operacionesJson: input.operacionesJson,
    });
  } catch (e) {
    await limpiar();
    throw e;
